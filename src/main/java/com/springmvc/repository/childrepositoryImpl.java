...
public class childrepositoryImpl implements childrepository {
	...
	public void create(child baby, HttpServletRequest request) {
		...
		template.update(sql, id, baby.getName(), baby.getBirth(), baby.getDay(), baby.getHeight(), baby.getWeight(),
				baby.getHead(), baby.getGender());
		listChild.add(baby);
	...
	@Override
	public List<child> readAll(String sessionid) {
		String sql = "select * from child where id=?";
		listChild = template.query(sql, new Object[]{sessionid}, new ChildRowMapper());
	...
	public void update(child baby) {
		...
		template.update(sql, baby.getBirth(), baby.getDay(), baby.getHeight(), baby.getWeight(), baby.getHead(),
				baby.getGender(), baby.getNum());
	}
