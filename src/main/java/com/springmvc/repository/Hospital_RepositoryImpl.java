...
public class Hospital_RepositoryImpl implements Hospital_Repository {
	...
	public void create(hospital hp) {
		...
		jdbcTemplate.update(sql, hp.getAddr(), hp.getPostNo(), hp.getTelephone(), hp.getXPos(), hp.getYPos(),
				hp.getYadmNm());
	}
	...
	public void createReserve(reservation rv) {
		...
		jdbcTemplate.update(sql, rv.getId(), rv.getHospitalName(), rv.getName(), rv.getBirth(), rv.getDate(),
				rv.getTime());
	}
