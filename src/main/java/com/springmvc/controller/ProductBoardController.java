...
public class ProductBoardController {
	...
	// 제품 등록 폼 처리
	@PostMapping("/write")
	public String write(@RequestParam("id") String id, @RequestParam("name") String name,
			@RequestParam("price") int price, @RequestParam("brand") String brand,
			@RequestParam("category") String category, @RequestParam("link") String link,
		...
		productboard newProductBoard = new productboard(mb.getId(), newProduct.getProduct_id(),
				new SimpleDateFormat("yyyy/MM/dd HH:mm:ss").format(new Date()), 0, request.getRemoteAddr());
	...
	@ResponseBody
	public List<review> deleteReview(@RequestBody HashMap<String, Object> map) {
		int num = (int)map.get("num");
		int parents_num = (int)map.get("parents_num");
		System.out.println("delete:"+ num);
		service.deleteReview(num);	
		List<review> list = service.readReview(parents_num);
